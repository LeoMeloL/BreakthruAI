...
class BreakthruGUI:
    ...
    def evaluate_board(self, board, player):
        ...
        weight_flag_distance = 0.5  # Peso da distância da bandeira à linha de chegada

        # Inicializar contadores para peças de cada jogador e distância da bandeira
        ...
        for row in range(len(board)):
            for col in range(len(board[row])):
                if board[row][col] == 3:
                    flag_distance = len(board) - 1 - row


        # Calcular a pontuação com base nos pesos e nos contadores
        player1_score = weight_pieces * player1_pieces
        player2_score = weight_pieces * player2_pieces + weight_flag_distance * flag_distance
    ...
    def minimax(self, board, depth, maximizing_player):
        if depth == 0 or self.game_over():
            return self.evaluate_board(board, maximizing_player), None  # Retorna o valor de avaliação e nenhum movimento
        best_move = None  # Inicializa a variável para armazenar o melhor movimento
        if maximizing_player:
            max_eval = float('-inf')
            for row in range(len(board)):
                for col in range(len(board[row])):
                    if board[row][col] == maximizing_player:
                        for move in self.generate_moves(board,maximizing_player):
    ...
    def is_valid_move_All(self, board, col, row):
        valid_moves = []
        piece = board[row][col]
        player = 1 if piece == 1 else 2  # Defina o jogador com base no valor da peça
        player = 3 if piece == 3 else 2

        # Verifique os movimentos possíveis para uma peça de jogador 1
        ...
        if not self.selected_piece:
           ...
           #self.imprimir_matriz(matriz)          
            ...
            if player == 1:
                ...
                player2 = 2
            value, move = self.minimax(matriz, 2, player2)
            print(value)
            print(move)
            print(player2)
            matriz = self.make_move(matriz, move)
            self.update_pieces_positions(matriz)
